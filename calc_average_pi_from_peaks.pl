...
open OUT1, ">>average_pi_from_IoD_midpoints_5Mbp.txt" or die;
...
open my $fh2, '<', "pi_genome_coords.txt" or die; # This is a file containing pi values in 20kbp windows across the genome
...
while (my $line = <$fh1>) {
	...
	for (my $i=$peak_start; $i <= $peak_start+$max_distance; $i=$i+$window_size) { # To move from 0 to 5 mb away from the peak downstream, 20 kbp at a time
		...
		for my $window (@dxy_data) {
			...
			my $start = $elements[1];
			my $dxy = $elements[7]; # CHANGE FOR EACH SPECIES COMPARISON
