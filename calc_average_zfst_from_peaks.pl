...
open OUT1, ">>average_zfst_from_IoD_midpoints_5Mbp.txt" or die;
#open OUT2, ">>windows_within_1mb_downstream_pbs_peaks.txt" or die;

#print OUT1 "Distance\tzfst\tspecies_comparison\n";


open my $fh1, '<', "BbBv_large_IoDs_midpoints.txt" or die;# This file contains two columns, 1st is the chromosome, 2nd is the IoD midpoint position
open my $fh2, '<', "zfst_genome_coords.txt" or die;  # This is a file containing ZFST values in 20kbp windows across the genome
...
while (my $line = <$fh1>) {
	...
	for (my $i=$peak_start; $i <= $peak_start+$max_distance; $i=$i+$window_size) { # To move from 0 to 5 mb away from the peak downstream, 20 kbp at a time
		...
		for my $window (@dxy_data) {
			...
			my $start = $elements[1];
			my $dxy = $elements[4]; # CHANGE FOR EACH SPECIES COMPARISON
